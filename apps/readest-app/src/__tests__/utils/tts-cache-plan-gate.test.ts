...
describe('isTTSCacheAllowed (fork local-resource policy)', () => {
...
describe('isTTSCacheAllowed — customization unlock', () => {
  ...
  test('fork unlock also entitles a storage-only buyer for local TTS cache', () => {
    expect(isTTSCacheAllowed('purchase', false)).toBe(true);
  });
});
