...
const LanForm: React.FC = () => {
  ...
  const [pendingPeerConfirmation, setPendingPeerConfirmation] = useState<DiscoveredLanPeer | null>(
    null,
  );
  ...
  const acquireDiscoveryLock = useCallback(async () => {
    ...
    const pending = setMulticastLock(true);
    ...
    try {
      ...
      if (switchingActiveToken) {
        ...
        if (needsMulticastLock) {
          ...
          const pendingPromotionLock = setMulticastLock(true);
  ...
  const confirmPeerConnection = (peer: DiscoveredLanPeer): void => {
    if (appService) {
      setPendingPeerConfirmation(peer);
    } else {
      void handleConnect(peer);
    }
  };

  const confirmPendingPeerConnection = async () => {
    const peer = pendingPeerConfirmation;
    if (!peer) return;
    setPendingPeerConfirmation(null);
    await handleConnect(peer);
  };

  const pendingPeerConfirmationDialog = pendingPeerConfirmation ? (
    <ModalPortal>
      <Alert
        title={_('Confirm')}
        message={_('Connect to {{name}} at {{host}}:{{port}} and enable LAN Sync?', {
          name: pendingPeerConfirmation.name || _('Readest device'),
          host: pendingPeerConfirmation.host,
          port: pendingPeerConfirmation.port,
        })}
        onCancel={() => setPendingPeerConfirmation(null)}
        onConfirm={() => void confirmPendingPeerConnection()}
...
const LanPeerDiscovery: React.FC<{
  _: TranslationFunc;
