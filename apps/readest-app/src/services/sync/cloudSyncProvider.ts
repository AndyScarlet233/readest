...
export const getEnabledFileSyncBackends = (
  ...
  // LAN last: the home-network backend — local peers, no cloud plan involved.
  if (settings?.lan?.enabled) enabled.push('lan');
  return enabled;
};

/** Any CLOUD third-party backend switched on ('lan' excluded — see the gate). */
export const hasAnyThirdPartyEnabled = (settings: SystemSettings | null | undefined): boolean =>
  ...
  // so enabling it alone must not flip Readest Cloud's derived default off.
...
export interface CloudSyncGate {
   ...
   * for it, not as a silent fallback (#4959). LAN is never paused: it is a
   * local-network channel with no plan or quota attached.
   */
  paused: boolean;
}
...
export const resolveCloudSyncGate = (
  ...
  return {
    ...
    backends,
    paused: cloudBackends.length > 0 && !isCloudSyncAllowed(plan, customizationPurchased),
  };
};
