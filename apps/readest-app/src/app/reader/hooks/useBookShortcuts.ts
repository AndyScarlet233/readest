...
const useBookShortcuts = ({ sideBarBookKey, bookKeys }: UseBookShortcutsProps) => {
  ...
  const applyShortcutFontSize = (fontSize: number) => {
  if (!sideBarBookKey) return;
  const viewSettings = getViewSettings(sideBarBookKey);
